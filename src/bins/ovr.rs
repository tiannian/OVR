...
mod dev {
    ...
    impl Env {
        ...
        fn alloc_resources(&mut self, id: NodeId, kind: Kind) -> Result<()> {
            ...
            cfg["p2p"]["laddr"] =
                toml_value(format!("tcp://127.0.0.1:{}", ports.tm_p2p));
            cfg["rpc"]["laddr"] =
                toml_value(format!("tcp://127.0.0.1:{}", ports.tm_rpc));
        ...
        fn gen_genesis(&mut self) -> Result<()> {
            ...
            let gen = |genesis_file: String| {
                self.validator_nodes
                    .values()
                    .map(|n| {
                        TmConfig::load_toml_file(&format!(
                            "{}/config/config.toml",
                            &n.home
                        ))
                        .map_err(|e| eg!(e))
        ...
        fn write_cfg(&self) -> Result<()> {
            serde_json::to_vec(self).c(d!()).and_then(|d| {
                fs::write(format!("{}/config.json", &self.home), d).c(d!())
            })
        }
